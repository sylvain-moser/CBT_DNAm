---
title: "TMEM132D_expression_CBT_profil"
...
```{r}
print ("starting to run markdown")
```
...
# Find the probes annotated to the TMEM132D gene:
...
```{r}
time_df=exp_df %>% dplyr::select(NID,Timepoint,ILMN_1763612)  %>% 
...
plot_TMEM132D_CBT=ggplot(time_df,aes(x=Time,y=ILMN_1763612,group=NID,colour=NID))+geom_line()+
  scale_x_continuous(name="Timepoints",breaks=c(0,1,2),labels=c("BE","PE1h","PE24h"))+
  labs(title = "TMEM132D during Therapy",y="gene expression")+
...
plot_TMEM132D_postint_remission=ggplot(remission_df,aes(x=Time,y=ILMN_1763612,group=NID,colour=as.factor(remission)))+geom_line()+
  scale_x_continuous(name="Timepoints",breaks=c(0,1,2),labels=c("BE","PE1h","PE24h"))+
...
```{r}
t.test(remission_df %>% dplyr::filter(Time==0 & remission==0) %>% pull (ILMN_1763612),remission_df %>% dplyr::filter(Time==0 & remission==1) %>% pull (ILMN_1763612))
t.test(remission_df %>% dplyr::filter(Time==1 & remission==0) %>% pull (ILMN_1763612),remission_df %>% dplyr::filter(Time==1 & remission==1) %>% pull (ILMN_1763612))
t.test(remission_df %>% dplyr::filter(Time==2 & remission==0) %>% pull (ILMN_1763612),remission_df %>% dplyr::filter(Time==2 & remission==1) %>% pull (ILMN_1763612))
```

### color coded for the remission status and centered at 0 for BE:

```{r}
centered_time_df=time_df %>% group_by(NID) %>% group_map(~mutate(.,ILMN_1763612=ILMN_1763612  - as.numeric(.$ILMN_1763612[.$Timepoint=="b1"])),.keep=T) %>% bind_rows()
