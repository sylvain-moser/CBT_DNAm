...
if(file.exists(snakemake@params[["surrogate_variable_matrix"]])==FALSE){
  ...
  surrogate_matrix=get_surrogates_matrix(meth_df %>% dplyr::select(contains("cg")) ,scale = F,n_var = 1000)
...
library(pbmcapply)
if (start_idx==0){
  print("analyzing snps 2 to 5001")
  results=pbmcapply::pbmclapply(colnames(meth_df)[2:5001],find_regulated_islands,sur_mat=surrogate_matrix)
  } else{
      stop("start_idx not equal to zero")
  }
