...
did_not_return_therapy=function(cpgs_list,residuals=TRUE,methylation_data,residuals_data){
  ...
  t0NID=meth_df %>% filter (Timepoint=="T0") %>% pull (NID)
  t1NID=meth_df %>% filter (Timepoint=="E") %>% pull (NID)
  
  t0df= meth_df %>% filter (Timepoint=="T0") %>% filter(NID %in% intersect(t0NID,t1NID)) %>% arrange(NID)
  t1df= meth_df %>% filter (Timepoint=="E")%>% filter(NID %in% intersect(t0NID,t1NID)) %>% arrange(NID)
