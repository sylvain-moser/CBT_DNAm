...
This markdown reports the main results of the analysis of methylation changes during the therapy phase of the Cognitive Behavioural Therapy as reported in Moser et al. 
...
therapy_final=final_processed_results
```


```{r}
therapy_signif_cpgs=therapy_final %>% filter (qval <=0.2)
sprintf("A total number of %s significantly differentially methylated CpG islands were found at a FDR threshold of 0.2",dim(therapy_signif_cpgs)[1])
```
...
```{r}
load(params$therapy_cpg_ranked)
load(params$therapy_cpg_ranked_anotated)
```
...
Using 4 different models for each CpGs allows us to get deeper insights into the possible mechanisms of methylation regulation during the therapy. It is in particular interesting to distinguish CpGs which undergo a linear change in methylation and CpGs undergoing a non-linear change. To this end we look at the distribution of the chosen model in the best ranked CpGs:
...
We observe that most of those CpGs are linearly regulated, which is coherent with the gradual nature of the therapy process. 
...
t_test_results=did_not_return_therapy(sum_ranked_cpgs[1:100, "island"],residuals=TRUE,methylation_data=params$methylation_df,residuals_data=params$methylation_residuals_df)
...
```{r,message=FALSE,warning=FALSE}
t_test_results=did_not_return_therapy(sum_ranked_cpgs[1:1000, "island"],residuals=TRUE,methylation_data=params$methylation_df,residuals_data=params$methylation_residuals_df)
diff_CpGs=t_test_results %>% filter (qval_t_test < 0.05)
...
missMethyl_ORA=function(signif_cpgs,all_cpgs,db,FDR_threshold){
  ...
  if (dim(signif_ORA)[1]==0){
    ...
    print(sprintf("There are %s significant Gene sets. Displaying only the %s most significant ones",dim(signif_ORA)[1],10))
  }
}
```
...
missMethyl_ORA(signif_cpgs = sum_ranked_cpgs$island[1:100],all_cpgs = therapy_final$island,db = "GO",FDR_threshold = 0.2)
missMethyl_ORA(signif_cpgs = sum_ranked_cpgs$island[1:100],all_cpgs = therapy_final$island,db = "KEGG",FDR_threshold = 0.2)
```
...
```{r,warning=FALSE,message=F,error=TRUE}
missMethyl_ORA(signif_cpgs = diff_CpGs_100$island,all_cpgs = therapy_final$island,db = "GO",FDR_threshold = 0.2)
missMethyl_ORA(signif_cpgs = diff_CpGs_100$island,all_cpgs = therapy_final$island,db = "KEGG",FDR_threshold = 0.2)
```
